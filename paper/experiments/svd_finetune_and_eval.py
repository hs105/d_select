"""
SVD Compress + Fine-tune + Save + Downstream Eval for Mistral-7B
================================================================
Full pipeline:
  1. Load Mistral-7B
  2. SVD compress W_K to target rank (on GPU)
  3. Fine-tune QK projections on WikiText-103
  4. Save the fine-tuned model as HuggingFace format
  5. Run downstream evaluation via lm-eval-harness

Usage:
  # Rank 256 (75% K cache saved) -- the paper's main result
  CUDA_VISIBLE_DEVICES=0 python svd_finetune_and_eval.py --rank 256 --device cuda:0

  # Also run the control (no compression, same fine-tuning)
  CUDA_VISIBLE_DEVICES=4 python svd_finetune_and_eval.py --rank 1024 --device cuda:0
"""
...
def load_wikitext103_split(tokenizer, split, seq_len=2048, max_tokens=None):
    """Load WikiText-103 split via HuggingFace datasets."""
...
def main():
    ...
    tag = f"r{args.rank}_ft" if is_compressed else "control_ft"

    print("=" * 70)
    print(f"SVD Compress (rank={args.rank}) + Fine-tune + Downstream Eval")
    print("=" * 70)
    ...
    val_ids = load_wikitext103_split(
        tokenizer, 'validation', args.seq_len)
    ...
    # Fine-tune
    print(f"\nFine-tuning for {args.epochs} epochs...", flush=True)
    print("-" * 70)
