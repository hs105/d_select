...
\begin{enumerate}
    \item \textbf{A theoretical and empirical analysis showing that attention selection is inherently low-dimensional.} We demonstrate that queries and keys require only $\BigO(\log N)$ dimensions, validated across positional selection (1~dim/head), content-based retrieval ($\log_2 N$~dims), and language modeling ($\dmodel/4$ incurs only 4.3\% PPL cost), with consistent results across three architectures (vanilla transformer, LLaMA, and Mistral with GQA) and scales from 10M to 7B parameters.

    \item \textbf{Asymmetric attention: a simple, drop-in modification} that decouples QK and V dimensionality. On WikiText-103, $\dselect = \dmodel/4$ reduces QK parameters by 75\% at 4.3\% perplexity cost. Post-training SVD of pretrained GPT-2 reveals keys to be far more compressible than queries, and lightweight QK fine-tuning (3 epochs on $<$1\% of pretraining data) recovers nearly all quality loss, reducing the residual gap to $<$2\%.

    \item \textbf{Significant KV cache savings for LLM inference.} The key cache shrinks proportionally with $\dselect$, reducing total KV cache by 37.5\% when $\dselect = \dmodel/4$. We provide three deployment paths---training from scratch, SVD + QK fine-tuning, and zero-cost SVD---and show composability with KV cache quantization for up to 16$\times$ combined compression. For a 7B-parameter model at 128K context, this saves 25\,GB per user.
\end{enumerate}
...
\subsection{Post-Training Compression via Factored Keys}

Asymmetric attention can also be \emph{retrofitted} to pretrained models via low-rank approximation of the key projection. Given a pretrained weight matrix $W_K \in \mathbb{R}^{\dmodel \times \dmodel}$, we compute its truncated SVD:
\begin{equation}
...
The result is mathematically equivalent to training with $\dselect = r$ from the start: queries and keys are $r$-dimensional, values remain at $\dmodel$, and the key cache stores only $r$ dimensions per token. No retraining is required; the entire conversion is a one-time factorization of $W_K$ per layer.
...
We have shown that attention selection (QK) and value transfer (V) have fundamentally different dimensionality requirements. Selection operates in $\BigO(\log N)$ dimensions, while value transfer requires the full model dimensionality. This asymmetry, confirmed across algorithmic tasks, language modeling from 10M to 7B parameters, and three architectures (vanilla transformer, LLaMA, and Mistral with GQA), motivates \textbf{asymmetric attention}: a simple modification that reduces QK dimensionality while preserving V dimensionality.

The practical benefit centers on KV cache reduction during inference---the dominant memory bottleneck in long-context LLM serving. SVD compression of key projections followed by lightweight QK fine-tuning achieves 75\% key cache savings at $\sim$2\% quality cost, validated on both GPT-2 (124M) and Mistral-7B (7.2B)---a 58$\times$ scale range. For production serving at 128K context, this frees 25\,GB per user, enabling $\sim$60\% more concurrent users on identical hardware.

We provide three deployment paths: (1) training from scratch with thin keys for maximum benefit; (2) SVD compression of existing models followed by lightweight QK fine-tuning, which recovers nearly all quality loss (from +17\% to +2.0\% on Mistral-7B, from +28\% to +1.8\% on GPT-2) using a small fraction of original pretraining data; and (3) zero-cost SVD compression at half rank for immediate savings with no retraining. The approach is orthogonal to both GQA (reducing head count) and KV cache quantization (reducing bit width), enabling up to 16$\times$ combined key cache compression.
