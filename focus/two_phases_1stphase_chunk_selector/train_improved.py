...
def compute_reward_batch(cfg, lm_model, lm_tokenizer, x_selected_batch, vocab_inv, questions, answers, device):
    """
    CONTRASTIVE REWARD: logP(answer | chunks + question) - logP(answer | question)
    Isolates the chunk contribution from the LM's parametric knowledge.
    """
    B = x_selected_batch.shape[0]
    context_with_chunks = []
    context_without_chunks = []
    target_tokens = []
    ...
    for i in range(B):
        ...
        if len(selected_tokens) == 0:
            context_with = questions[i]
        else:
            context_with = " ".join(selected_tokens) + " " + questions[i]
        
        context_with_chunks.append(context_with)
        context_without_chunks.append(questions[i])
        target_tokens.append(answers[i])
    
    logp_with = get_next_token_logprob_batch(lm_model, lm_tokenizer, context_with_chunks, target_tokens, device)
    logp_without = get_next_token_logprob_batch(lm_model, lm_tokenizer, context_without_chunks, target_tokens, device)
...
def main():
    ...
    logger = Logger(cfg.log_dir, "contrastive_reward")
    ...
    logger.log(f"\nTraining {cfg.train_steps} steps with CONTRASTIVE reward...")
