...
class Config:
    ...
    # NEW: Better prompt construction
    query_template: str = "Based on this information: {context}\n\nQuestion: What is the capital city of France?\nAnswer:"
...
def compute_reward_with_lm(cfg: Config, lm_model, lm_tokenizer, 
    ...
    for i in range(B):
        ...
        if len(selected_tokens) == 0:
            context_text = "No information provided."
        else:
            context_text = " ".join(selected_tokens)
        
        # Use template for better prompt structure
        full_prompt = cfg.query_template.format(context=context_text)
        
        log_prob = get_next_token_logprob(lm_model, lm_tokenizer, full_prompt, cfg.target_answer, device)
...
def main():
    ...
    for i, (sent, tokens) in enumerate(zip(sentences, all_tokens)):
        ...
        print(f"Keyword chunks: {kw_chunks}")
        for c in range(cfg.C):
            start = c * cfg.L
            end = min(start + cfg.L, len(tokens))
            chunk_toks = tokens[start:end]
            has_kw = cfg.keyword in chunk_toks
            marker = " ← HAS PARIS" if has_kw else ""
            print(f"  Chunk {c}: {chunk_toks}{marker}")

    # Test baseline (no context)
    print("\n" + "=" * 70)
    print("BASELINE TEST")
    print("=" * 70)
    baseline_prompt = cfg.query_template.format(context="No information provided.")
    baseline_logprob = get_next_token_logprob(lm_model, lm_tokenizer, baseline_prompt, cfg.target_answer, device)
        ...
        if step % cfg.log_every == 0 or step == 1:
            with torch.no_grad():
                ...
                for i in range(len(sentences)):
                    ...
                    selected = sorted(greedy[i].tolist())
                    overlap = len(set(selected) & set(kw_chunks))
                    print(f"  S{i+1}: greedy={selected} r={r_g[i].item():.3f} overlap={overlap}/{len(kw_chunks)}")
    ...
    with torch.no_grad():
        ...
        for i in range(len(sentences)):
            ...
            print(f"  Policy selected: {sorted(greedy[i].tolist())}")
            overlap = len(set(greedy[i].tolist()) & set(kw_chunks))
            print(f"  Overlap: {overlap}/{len(kw_chunks)}")
