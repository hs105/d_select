...
def compress_model_qk(model, rank, mode='both', verbose=True):
    """
    Compress Q and/or K projections in all layers via SVD.

    mode: 'both' = compress Q and K
          'k_only' = compress only K (Q stays full rank)
          'q_only' = compress only Q (K stays full rank)
          'qkv' = compress Q, K, and V
    ...
    for i in range(n_layers):
        ...
        # Compress Q
        if mode in ('both', 'q_only', 'qkv'):
            W_Q, q_err, _, _ = svd_compress_weight(W_Q, rank)

        # Compress K
        if mode in ('both', 'k_only', 'qkv'):
            W_K, k_err, _, _ = svd_compress_weight(W_K, rank)

        # Compress V
        if mode == 'qkv':
            W_V, v_err, _, _ = svd_compress_weight(W_V, rank)
        ...
        # Write back
        set_qkv_weights(model, i, W_Q, W_K, W_V, b_Q, b_K, b_V)

        if verbose and (i == 0 or i == n_layers - 1):
            print(f"  Layer {i}: Q_err={q_err:.4f}, K_err={k_err:.4f}, V_err={v_err:.4f}")

    n_compressed = {'both': 2, 'k_only': 1, 'q_only': 1, 'qkv': 3}[mode]
    stats = {
        'rank': rank,
        'mode': mode,
        'avg_q_error': total_q_error / n_layers if mode in ('both', 'q_only', 'qkv') else 0.0,
        'avg_k_error': total_k_error / n_layers if mode in ('both', 'k_only', 'qkv') else 0.0,
        'avg_v_error': total_v_error / n_layers if mode == 'qkv' else 0.0,
        'original_qk_params': 2 * d_model * d_model * n_layers,
        'compressed_qk_params': n_compressed * (d_model * rank + rank * d_model) * n_layers,
    }
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='both',
                        choices=['both', 'k_only', 'q_only', 'qkv'],
                        help='What to compress: both=QK, k_only=K, q_only=Q, qkv=all')
    ...
    print("=" * 70)
    print(f"COMPRESSION SWEEP: mode={args.mode}")
    print("=" * 70)
    ...
    for rank in ranks:
        ...
        print(f"--- Rank {rank} (d_select/head = {rank // n_heads}, mode={args.mode}) ---")

        # Reload fresh model each time
        model, _ = load_model_and_tokenizer(args.model, device)

        # Compress
        stats = compress_model_qk(model, rank, mode=args.mode, verbose=True)

        # Evaluate
        loss, ppl = evaluate_perplexity(model, eval_ids, device, args.batch_size)
        ppl_change = (ppl - base_ppl) / base_ppl * 100

        # KV cache savings (only K compression matters for cache)
        original_kv_per_token = 2 * d_model * n_layers
        if args.mode in ('both', 'k_only'):
            compressed_kv_per_token = (rank + d_model) * n_layers  # small K + full V
        else:
            compressed_kv_per_token = original_kv_per_token  # Q compression doesn't help cache
    ...
    # Also run comparison modes if sweeping
    if args.rank is None and args.mode == 'both':
        comparison_rank = d_model // 4  # 192 for GPT-2
        print("=" * 70)
        print(f"COMPARISON at rank={comparison_rank}: K-only vs Q-only vs Both")
        print("=" * 70)

        for cmp_mode in ['k_only', 'q_only']:
            print(f"\n--- Rank {comparison_rank}, mode={cmp_mode} ---")
            model, _ = load_model_and_tokenizer(args.model, device)
            stats = compress_model_qk(model, comparison_rank, mode=cmp_mode, verbose=True)
            loss, ppl = evaluate_perplexity(model, eval_ids, device, args.batch_size)
            ppl_change = (ppl - base_ppl) / base_ppl * 100
            print(f"  PPL: {ppl:.2f} ({ppl_change:+.1f}%)")
            ...
            results.append({
                ...
                'rank_per_head': comparison_rank // n_heads,
                'mode': cmp_mode,
                'ppl': ppl,
                'loss': loss,
                'ppl_change_pct': ppl_change,
                'avg_q_error': stats['avg_q_error'],
                'avg_k_error': stats['avg_k_error'],
                'kv_cache_savings_pct': 0 if cmp_mode == 'q_only' else
                    (1 - (comparison_rank + d_model) * n_layers / (2 * d_model * n_layers)) * 100,
            })
